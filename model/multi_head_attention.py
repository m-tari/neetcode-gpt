...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.heads = nn.ModuleList(
            [SingleHeadAttention(embedding_dim, attention_dim // num_heads) for _ in range(num_heads)]
        )
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        outputs = torch.cat([head(embedded) for head in self.heads], dim=2)
        return torch.round(self.output_projection(outputs), decimals=4)
